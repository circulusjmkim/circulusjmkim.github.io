const MENUS = [
  ...
  },
  {
    value: 'robot',
    label: '로봇',
    desc: '로봇과 관련된 일부 기능들을 관리자가 직접 수행할 수 있습니다.',
    list: [
      {value: 'robot_disconn', label: '로봇 연결 해제', desc: '로봇의 물리적 또는 논리적 아이디로 해당 로봇을 owner와 연결 해제합니다.' },
      {value: 'robot_data_transfer', label: '로봇 데이터 이전', desc: '로봇의 시리얼 번호가 변경되었을 경우 새로운 시리얼로 기존 정보를 이용할 수 있도록 데이터를 이전합니다.' },
      {value: 'robot_data_clear', label: '로봇 데이터 클리어', desc: '로봇의 물리적 또는 논리적 아이디와 관련된 모든 데이터(설치된 bot 정보, bot의 모든 연관 데이터(Optional))를 삭제합니다.' },
    ]
  },
  {
    value: 'user',
    label: '사용자',
    desc: '사용자와 관련된 일부 기능들을 관리자가 직접 수행할 수 있습니다.',
    list: [
      {value: 'user_update_pw', label: '사용자 비밀번호 변경', desc: '사용자의 물리적 또는 논리적 아이디로 해당 사용자의 비밀번호를 변경합니다.'},
      {value: 'user_update_info', label: '사용자 인증정보 변경', desc: '사용자의 물리적 또는 논리적 아이디로 해당 사용자의 인증정보(tel, email) 정보를 변경합니다.'},
      {value: 'user_delete_info', label: '사용자 데이터 클리어', desc: '사용자의 물리적 또는 논리적 아이디와 관련된 모든 데이터(로봇, 로봇에 설치된 bot 정보, 해당 로봇 bot의 모든 연관 데이터(Optional))를 삭제합니다.'},
      {value: 'user_update-withdraw', label: '사용자 탈퇴 정보 삭제 및 백업', desc: '한달 간격으로 이루어지는 탈퇴 회원의 정보 백업을 탈퇴 회원 정보로 조회하여 즉시 실행합니다.'},
      {value: 'user_signup', label: '사용자 회원가입', desc: '최소한의 정보로 사용자 정보를 생성합니다.'},
    ]
  },
  {
    value: 'redis',
  ...
  },
  {
    ...
    list: [
      ...
      {value: 'notice_update', label: '공지사항 관리', desc: '모바일에 보여질 공지사항을 수정, 삭제, 노출 순서 등을 변경합니다.'},
    ]
  }
];
